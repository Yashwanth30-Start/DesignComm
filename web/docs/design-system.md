...
5. Any panel/circuit view renders through one of exactly two canonical components — never a hand-rolled table: `PanelSchedule` (`components/ui/PanelSchedule.tsx`) for the simple flat grid, and `EnergizedPanelBoard` (`features/panels/EnergizedPanelBoard.tsx`) for the interactive two-bank breaker board (PJKS-style: metadata strip, Energized/Future/Spares/Schedule tabs, merged multi-pole cells, green energized highlighting, and the `CircuitDrawer` that shows every connected app's records for a pressed circuit). `EnergizedPanelBoard` supersedes the old `LivePanelSchedule`; its circuits come from `features/panels/panel-board.ts` (`boardFromRecords` for imported SharePoint records, `boardFromMock` for fixtures). Extend these components if a new view needs different behavior.
