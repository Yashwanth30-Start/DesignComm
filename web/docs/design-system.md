...
- **`features/landing`** — CinematicLanding (the scroll-driven landing: raw video hero → black gallery panel with the TesseractSwarm flowing behind scatter-grid cards → white outro with pill CTA; all RAF-driven, no scroll-snap), plus its primitives FadeIn, AnimatedHeading (char-stagger entrance), CustomCursor (desktop exclusion cursor). `.liquid-glass` and `.bp-card` CSS live in `app/globals.css`.
